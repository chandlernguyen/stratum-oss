...
because the failure mode is "a new table forgot". Run all three against a real
Postgres before shipping a migration; the `db` job in `.github/workflows/ci.yml`
wires them up (the workflow is disabled on this repository).
