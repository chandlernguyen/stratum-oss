...
- There is **no support commitment**. Issues may go unanswered.
- It is published to be read and learned from, and to be a working example of a
  multi-tenant agentic application.
...
Because the project is unmaintained, a clear report is more useful than a fast
one, and a reproducible report may still not be actioned.
...
- **Never commit secrets.** `.env` files are gitignored for a reason; use the
  ...
  `bash scripts/oss/leak-gate.sh --tracked` before pushing — it is also a CI job,
  and it checks for credentials, absolute machine paths, author identity in code
