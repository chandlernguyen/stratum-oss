...
**Run the leak gate before every commit and push:**
`bash scripts/oss/leak-gate.sh --tracked`. CI is disabled on this repository, so
nothing enforces it automatically — and this repository is public, so a
credential is compromised the moment it is pushed, even if it is deleted
afterwards.

`supabase/seed.sql` creates accounts with known credentials and is for local use
only.
