...
# README and LICENSE are where attribution belongs, so they may name the author's
# domain. `grep -rIn` prefixes each hit with its path, so the exemption must
# allow that prefix rather than anchoring at the start of the line.
