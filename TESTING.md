...
`.github/workflows/ci.yml` runs on every push and pull request, plus on demand.
It is deliberately small — two fast, hermetic jobs:

| Job | What it runs |
|---|---|
| `backend` | 129 tests across 9 modules; no Supabase, no API, no Postgres, no network |
| `web` | 31 Vitest tests across 4 files: auth store, PKCE, locales, auth callback |

Both must pass. There is no advisory tier and no Docker: the integration and E2E
suites need Supabase, several minutes and image pulls, which is a poor trade for
every push on a v1 repository. Run those before a release using the commands
earlier in this document.

The point of this workflow is narrow and worth stating: it stops the suites from
silently rotting the way they did before, when the unit layer could not run at
all and nobody noticed.

### Why the backend job is configured the way it is

- `SUPABASE_URL` and `API_BASE_URL` point at a **closed port** (`127.0.0.1:9`).
  If one of these tests starts depending on a live service, it fails here rather
  than passing in CI and breaking for a contributor.
- `CLOUDSDK_CONFIG` points at an empty directory, so Application Default
  Credentials are invisible — the way they are on a fresh machine. This is not
  incidental: it is what caught `rag_service.py` constructing a Cloud Storage
  client at import time. On a developer machine with gcloud configured, that test
  passed and hid the bug.

### The backend test list

`web` now runs a bare `vitest run`; every unit test passes. The backend job
names the modules that need no services; extend that list as more tests become
hermetic.
