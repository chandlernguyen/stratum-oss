...
- **Not actively developed.** No roadmap, no release schedule.
- **No support commitment.** Issues may go unanswered.
- **A reference, not a product.** Read it, learn from it, fork it. Do not depend
  on it.

It is published because a working multi-tenant agentic application is a more
useful artefact than an architecture diagram, and because the decisions that were
hard to get right are the ones worth sharing. `ARCHITECTURE.md` covers the design;
this file covers running it.
...
`tests/automated/test_tenant_privilege_contract.py` asserts this, and CI runs it
...
CI runs the hermetic unit layers and a database job for the tenant-isolation
tests. The integration and E2E suites need Docker and several minutes, so run
